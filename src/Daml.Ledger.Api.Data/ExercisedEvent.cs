...
namespace Com.Daml.Ledger.Api.Data 
{
    public class ExercisedEvent : TreeEvent // Doesn't derive from IEvent as not represented by Event type in base API, instead it has it's own type in the base API : 'ExercisedEvent'
    {
        ...
        public DigitalAsset.Ledger.Api.V1.ExercisedEvent ToProto()
        {
            var exercisedEvent = new DigitalAsset.Ledger.Api.V1.ExercisedEvent { EventId = EventId, Choice = Choice, ChoiceArgument = ChoiceArgument.ToProto(), Consuming = Consuming,
        ...
        public static ExercisedEvent FromProto(DigitalAsset.Ledger.Api.V1.ExercisedEvent exercisedEvent)
        {
