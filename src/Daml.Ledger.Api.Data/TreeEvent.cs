...
namespace Com.Daml.Ledger.Api.Data 
{
    public abstract class TreeEvent : IComparable<TreeEvent>, IEquatable<TreeEvent>
    {
        ...
        public DigitalAsset.Ledger.Api.V1.TreeEvent ToProtoTreeEvent()
        {
            if (this is CreatedEvent)
                return new DigitalAsset.Ledger.Api.V1.TreeEvent { Created = ((CreatedEvent) this).ToProto()};

            if (this is ExercisedEvent) 
                return new DigitalAsset.Ledger.Api.V1.TreeEvent { Exercised = ((ExercisedEvent) this).ToProto() };

            throw new ApplicationException($"this should be CreatedEvent or ExercisedEvent, found {ToString()}");
        }

        public static TreeEvent FromProtoTreeEvent(DigitalAsset.Ledger.Api.V1.TreeEvent treeEvent)
        {
            if (treeEvent.KindCase == DigitalAsset.Ledger.Api.V1.TreeEvent.KindOneofCase.Created)
                return CreatedEvent.FromProto(treeEvent.Created);
            
            if (treeEvent.KindCase == DigitalAsset.Ledger.Api.V1.TreeEvent.KindOneofCase.Exercised)
