...
namespace Com.Daml.Ledger.Api.Data 
{
    public class DamlTextMap : Value
    {
        ...
        {
            var mb = new DigitalAsset.Ledger.Api.V1.Map();
            foreach (var pair in Map)
                mb.Entries.Add(new Com.DigitalAsset.Ledger.Api.V1.Map.Types.Entry { Key = pair.Key, Value = pair.Value.ToProto() });

            return new DigitalAsset.Ledger.Api.V1.Value { Map = mb };
        }

        public static DamlTextMap FromProto(DigitalAsset.Ledger.Api.V1.Map map)
        {
