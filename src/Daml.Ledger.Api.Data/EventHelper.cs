...
namespace Daml.Ledger.Api.Data
{
    ...
    {
        public static Com.Daml.Ledger.Api.V1.Event ToProtoEvent(IEvent theEvent)
        {
            if (theEvent is ArchivedEvent)
                return new Com.Daml.Ledger.Api.V1.Event { Archived = ((ArchivedEvent) theEvent).ToProto() };

            if (theEvent is CreatedEvent)
                return new Com.Daml.Ledger.Api.V1.Event { Created = ((CreatedEvent) theEvent).ToProto() };
