 ...
 *
 * Changed the namespace and added public string ToPlainString()
 */
...
namespace Daml.Ledger.Api.Data.Util
{
    ...
    public class BigDecimal : IComparable, IComparable<BigDecimal>, IEquatable<BigDecimal>, IConvertible
    {
        ...
        /// <summary>
        /// Return a string representing the BigDecimal value, not using exponential notation
        /// </summary>
        /// <returns></returns>
        public string ToPlainString()
        {
            ...
            {
                int coefficientLength = sb.Length;
                int negOffset = 0;
                if (_coeff.IsNegative)
                {
                    coefficientLength--;
                ...
                int numDec = -_exp;
                if (numDec < coefficientLength)
                    sb.Insert(coefficientLength - numDec + negOffset, '.');

                else if (numDec == coefficientLength)
                    sb.Insert(negOffset, "0.");

                else
                {
                    int numZeros = numDec - coefficientLength;
                    sb.Insert(negOffset, "0", numZeros);
                    sb.Insert(negOffset, "0.");
                }
            }
