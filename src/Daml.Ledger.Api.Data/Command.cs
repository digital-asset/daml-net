...
namespace Com.Daml.Ledger.Api.Data 
{
    public abstract class Command : IComparable<Command>, IEquatable<Command>
    {
        ...
        public static Command FromProtoCommand(DigitalAsset.Ledger.Api.V1.Command command)
        {
            switch (command.CommandCase)
            {
                case DigitalAsset.Ledger.Api.V1.Command.CommandOneofCase.Create:
                    return CreateCommand.FromProto(command.Create);
                case DigitalAsset.Ledger.Api.V1.Command.CommandOneofCase.Exercise:
                    return ExerciseCommand.FromProto(command.Exercise);
                case DigitalAsset.Ledger.Api.V1.Command.CommandOneofCase.CreateAndExercise:
                    return CreateAndExerciseCommand.FromProto(command.CreateAndExercise);
                case DigitalAsset.Ledger.Api.V1.Command.CommandOneofCase.ExerciseByKey:
                    return ExerciseByKeyCommand.FromProto(command.ExerciseByKey);
                case DigitalAsset.Ledger.Api.V1.Command.CommandOneofCase.None:
                default:
                    throw new ProtoCommandUnknown(command);
            }
        }

        public DigitalAsset.Ledger.Api.V1.Command ToProtoCommand()
        {
            DigitalAsset.Ledger.Api.V1.Command command = new DigitalAsset.Ledger.Api.V1.Command();
    ...
    {
        public ProtoCommandUnknown(DigitalAsset.Ledger.Api.V1.Command command)
         : base($"Command unknown {command}")
        { }
    }
} 
