...
namespace Com.Daml.Ledger.Api.Data 
{
    public class InclusiveFilter : Filter
    {
        ...
        public override DigitalAsset.Ledger.Api.V1.Filters ToProto()
        {
            var inclusiveFilter = new DigitalAsset.Ledger.Api.V1.InclusiveFilters();
            inclusiveFilter.TemplateIds.AddRange(from id in TemplateIds select id.ToProto());

            return new DigitalAsset.Ledger.Api.V1.Filters { Inclusive = inclusiveFilter };
        }

        public static InclusiveFilter FromProto(DigitalAsset.Ledger.Api.V1.InclusiveFilters inclusiveFilters) => new InclusiveFilter(from templateId in inclusiveFilters.TemplateIds select Identifier.FromProto(templateId));
