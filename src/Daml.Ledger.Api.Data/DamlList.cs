...
namespace Com.Daml.Ledger.Api.Data
{
    public class DamlList : Value
    {
        ...
        public override DigitalAsset.Ledger.Api.V1.Value ToProto()
        {
            var list = new DigitalAsset.Ledger.Api.V1.List();
            list.Elements.AddRange(from v in Values select v.ToProto());
            return new DigitalAsset.Ledger.Api.V1.Value { List = list };
        }

        public static DamlList FromProto(DigitalAsset.Ledger.Api.V1.List list) => new DamlList(new List<Value>(from v in list.Elements select Value.FromProto(v)));
