...
(use dotnet run -c ReleaseNuget to run the release configuration which uses nuget references - so ensure that the whole Daml.Ledger solution has first been built with `dotnet build -c ReleaseNuget`)
```
